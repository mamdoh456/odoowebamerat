odoo.define('acs_hms_online_appointment.acs_hms_online_appointment', function (require) {
    ...
    $("#AcsRecordSearch").on('keyup', function() {
        ...
        records = document.getElementsByClassName("acs_coiffure_block");
        for (i = 0; i < records.length; i++) {
            rec = records[i].getElementsByClassName("acs_coiffure_name")[0];
            ...
            var coiffures = $(this).parents().find('.appoint_person_panel:visible');
            if (coiffures.length) {
                coiffures[0].click();
            }
        }
    ...
    $('.acs_appointment').on('change', "input[name='appoitment_by']", function () {
        ...
        if (appoitment_by.val()=='department') {
            ...
            $coiffure_datas.removeClass('acs_hide');
            var coiffures = $(this).parents().find('.appoint_person_panel');
            if (coiffures.length) {
                coiffures[0].click();
            }
        }
