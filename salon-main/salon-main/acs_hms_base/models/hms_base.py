...
class ACSHmsMixin(models.AbstractModel):
    ...
    def acs_prepare_invocie_data(self, partner, patient, product_data, inv_data):
        ...
        data = {
            ...
            'coiffure_id': inv_data.get('coiffure_id', False),
            'salon_invoice_type': inv_data.get('salon_invoice_type', False),
            'fiscal_position_id': fiscal_position_id,
        }
        if inv_data.get('ref_coiffure_id', False):
            data['ref_coiffure_id'] = inv_data.get('ref_coiffure_id', False)
...
class ACSDocumntMixin(models.AbstractModel):
    ...
    attachment_ids = fields.Many2many('ir.attachment', 'attachment_acs_hms_rel', 'record_id', 'attachment_id',
                                      compute="_acs_attachemnt_count", string="Attachments")
