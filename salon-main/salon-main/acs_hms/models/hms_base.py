...
class ResPartner(models.Model):
    ...
    is_referring_doctor = fields.Boolean(string="Is Refereinng Coiffure")


class ResUsers(models.Model):
    _inherit= "res.users"

    @api.depends('coiffure_ids')
    def _compute_coiffure_count(self):
        for user in self.with_context(active_test=False):
            user.coiffure_count = len(user.coiffure_ids)
    ...
    coiffure_count = fields.Integer(string="#coiffure", compute="_compute_coiffure_count")
    coiffure_ids = fields.One2many('hms.coiffure', 'user_id', string='Related coiffure')
    patient_count = fields.Integer(string="#Patient", compute="_compute_patient_count")

    def action_create_coiffure(self):
        self.ensure_one()
        self.env['hms.coiffure'].create({
            'user_id': self.id,
            'name': self.name,
        })
...
class salonDepartment(models.Model):
    ...
    department_type = fields.Selection([('general','General')], string="salon Department")
...
class ACSMedicalAlert(models.Model):
    ...
    _description = "Alert for Patient"
...
class product_template(models.Model):
    ...
    salon_product_type = fields.Selection(selection_add=[('procedure', 'Procedure'), ('consultation','Consultation')])
