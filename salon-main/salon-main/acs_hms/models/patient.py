...
class ACSPatient(models.Model):
    ...
    ref_doctor_ids = fields.Many2many('res.partner', 'rel_doc_pat', 'doc_id', 
        'patient_id', 'Referring Coiffures', domain=[('is_referring_doctor','=',True)])

    #Diseases
    medical_history = fields.Text(string="Past History")
    ...
    def create_invoice(self):
        ...
        invoice = self.acs_create_invoice(partner=self.partner_id, patient=self, product_data=[{'product_id': product_id}], inv_data={'salon_invoice_type': 'patient'})
        self.invoice_id = invoice.id

    def action_appointment(self):
        action = self.env["ir.actions.actions"]._for_xml_id("acs_hms.action_appointment")
        action['domain'] = [('patient_id','=',self.id)]
        action['context'] = {'default_patient_id': self.id, 'default_coiffure_id': self.primary_doctor.id}
        return action
...
class ACSFamilyMember(models.Model):
    ...
    patient_id = fields.Many2one('hms.patient', string='Client')
