...
class ACSPrescriptionOrder(models.Model):
    ...
    def _current_user_doctor(self):
        coiffure_id =  False
        ids = self.env['hms.coiffure'].search([('user_id', '=', self.env.user.id)])
        if ids:
            coiffure_id = ids[0].id
        return coiffure_id
    ...
    patient_id = fields.Many2one('hms.patient', ondelete="restrict", string='Client', required=True, states=READONLY_STATES, tracking=True)
    ...
    company_id = fields.Many2one('res.company', ondelete="cascade", string='salon',default=lambda self: self.env.user.company_id, states=READONLY_STATES)
    ...
    coiffure_id = fields.Many2one('hms.coiffure', ondelete="restrict", string='Prescribing Doctor',
...
class ACSPrescriptionLine(models.Model):
    ...
    product_id = fields.Many2one('product.product', ondelete="cascade", string='Product', required=True, domain=[('salon_product_type', '=', 'medicament')])
    ...
    company_id = fields.Many2one('res.company', ondelete="cascade", string='salon', related='prescription_id.company_id')
