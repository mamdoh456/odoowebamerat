...
class ACSPatientDisease(models.Model):
    ...
    coiffure_id = fields.Many2one('hms.coiffure', ondelete='restrict', string='Coiffure',
        help='Coiffure who treated or diagnosed the patient')
    is_allergy = fields.Boolean(string='Allergic Disease')
    pregnancy_warning = fields.Boolean(string='Pregnancy warning')
    patient_id = fields.Many2one('hms.patient', ondelete='cascade', string='Client')
...
class FamilyDiseases(models.Model):
    ...
    patient_id = fields.Many2one('hms.patient', ondelete='cascade', string='Client', index=True)
