...
class ACSTreatment(models.Model):
    ...
    coiffure_id = fields.Many2one('hms.coiffure', ondelete='restrict', string='coiffure',
        help='coiffure who treated or diagnosed the patient', states=READONLY_STATES, tracking=True)
    attending_coiffure_ids = fields.Many2many('hms.coiffure', string='Primary Coiffures',
    ...
    def create_invoice(self):
        ...
        invoice = self.acs_create_invoice(partner=self.patient_id.partner_id, patient=self.patient_id, product_data=[{'product_id': product_id}], inv_data={'salon_invoice_type': 'treatment'})
