...
class AppointmentCabin(models.Model):
    ...
    _description = "Appointment branch"

    name = fields.Char(string='Appointment branch', required=True, translate=True)
...
class Appointment(models.Model):
    ...
    patient_id = fields.Many2one('hms.patient', ondelete='restrict',  string='Client',
        required=True, index=True,help='Patient Name', states=READONLY_STATES, tracking=True)
    image_128 = fields.Binary(related='patient_id.image_128',string='Image', readonly=True)
    coiffure_id = fields.Many2one('hms.coiffure', ondelete='restrict', string='Coiffure',
        index=True, help='Coiffure\'s Name', states=READONLY_STATES, tracking=True)
    ...
    medical_advice = fields.Text(string='Advice', states=READONLY_STATES, help="The provision of a formal professional opinion regarding what a specific individual should or should not do to restore or preserve health.")
    chief_complain = fields.Text(string='Chief Complaints', states=READONLY_STATES, help="The concise statement describing the symptom, problem, condition, diagnosis, Coiffure-recommended return, or other reason for a medical encounter.")
    ...
    product_id = fields.Many2one('product.product', ondelete='restrict', 
        ...
        domain=[('salon_product_type', '=', "consultation")], required=True, 
    ...
    company_id = fields.Many2one('res.company', ondelete='restrict', states=READONLY_STATES,
        string='salon', default=lambda self: self.env.user.company_id.id)
    ...
    medical_history = fields.Text(related='patient_id.medical_history', 
        string="Past History", readonly=True)
    ...
    cabin_id = fields.Many2one('appointment.cabin', ondelete='cascade',
        string='Consultation Room (Cabin)', help="Appointment branch", states=READONLY_STATES, copy=False)
    ...
    ref_coiffure_id = fields.Many2one('res.partner', ondelete='restrict', string='Referring Coiffure',
        index=True, help='Referring Coiffure', states=READONLY_STATES)
    responsible_id = fields.Many2one('hms.coiffure', "Responsible Jr. Coiffure", states=READONLY_STATES)
    medical_alert_ids = fields.Many2many('acs.medical.alert', 'appointment_medical_alert_rel','appointment_id', 'alert_id',
        string='Alerts', related='patient_id.medical_alert_ids')
    ...
    def onchange_department(self):
        ...
        if self.department_id:
            coiffures = self.env['hms.coiffure'].search([('department_ids', 'in', self.department_id.id)])
            res['domain'] = {'coiffure_id':[('id','in',coiffures.ids)]}
    ...
    def action_refer_doctor(self):
        ...
        action['context'] = {'default_patient_id': self.patient_id.id, 'default_coiffure_id': self.coiffure_id.id}
    ...
    def action_create_evaluation(self):
        ...
        action['domain'] = [('patient_id','=',self.id)]
        action['context'] = {'default_patient_id': self.patient_id.id, 'default_coiffure_id': self.coiffure_id.id, 'default_appointment_id': self.id}
        return action
    ...
    def onchange_patient_id(self):
        if self.patient_id:
            ...
            #Avoid setting coiffure if already there from treatment or manually selected.
            if not self.coiffure_id:
                self.coiffure_id = self.patient_id.primary_doctor and self.patient_id.primary_doctor.id
            if appointment_id:
    ...
    @api.onchange('coiffure_id', 'department_id', 'consultation_type')
    def onchange_coiffure(self):
        product_id = False
        ...
        if self.coiffure_id:
            if self.coiffure_id.consultaion_service_id:
                product_id = self.coiffure_id.consultaion_service_id.id

            if self.coiffure_id.appointment_duration:
                self.planned_duration = self.coiffure_id.appointment_duration

            if self.consultation_type=='followup':
                if self.coiffure_id.followup_service_id:
                    product_id = self.coiffure_id.followup_service_id.id
        if product_id:
    ...
    def action_prescription(self):
        ...
        action['context'] = {
                'default_patient_id': self.patient_id.id,
                'default_coiffure_id': self.coiffure_id.id,
